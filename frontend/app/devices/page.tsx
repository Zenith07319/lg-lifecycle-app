      ...
      <div className="px-4 py-4">
        {ready && list.length === 0 ? (
          <div className="text-center py-20 px-6">
            <div className="w-16 h-16 rounded-3xl bg-accent-soft flex items-center justify-center mx-auto mb-4">
              <Snowflake size={30} className="text-accent" />
            </div>
            ...
            <Link href="/diagnose" className="inline-flex items-center gap-1.5 mt-5 rounded-2xl text-white font-extrabold px-5 py-3 text-[14px]" style={{ background: "linear-gradient(135deg,#047d86,#034349)" }}>
          ...
          <div className="space-y-3">
            {list.map((dev) => {
              ...
              return (
                <div key={dev.sessionId} className="rounded-[20px] bg-surface border border-line p-4 shadow-[var(--shadow-card)]">
                  ...
                  <div className="rounded-xl bg-accent-soft px-3 py-2 mt-3 text-[12px] text-ink-soft">
                    💡 1순위 추천 · <b className="text-accent">{dev.recommendation}</b>
                  </div>
                  <div className="flex gap-2 mt-3">
                    <Link href={`/result/${dev.sessionId}`} className="flex-1 flex items-center justify-center gap-1 rounded-xl text-white font-extrabold py-2.5 text-[13px]" style={{ background: "linear-gradient(135deg,#047d86,#034349)" }}>
            ...
            <Link href="/diagnose" className="flex items-center justify-center gap-1.5 rounded-2xl border border-dashed border-accent/40 text-accent font-extrabold py-3.5 text-[14px] mt-1">
