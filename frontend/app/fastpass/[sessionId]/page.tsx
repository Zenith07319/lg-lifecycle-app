...
export default function FastPassPage() {
  ...
  if (!data) return <div className="flex flex-col items-center py-32 text-muted gap-3"><Loader2 className="animate-spin text-crimson" size={28} /><p className="text-[14px]">불러오는 중…</p></div>;
  ...
  const Row = ({ k, v }: { k: string; v: string }) => (
    <div className="flex text-[12.5px] py-1.5 border-b border-dashed border-[#e6ddd6] last:border-0">
      <span className="w-24 shrink-0 text-[#8a7f84] font-semibold">{k}</span>
      <span className="flex-1 text-[#241c20] font-medium">{v}</span>
    </div>
  );
    ...
    <div>
      ...
      <div className="px-4 py-4">
        ...
        <div className="printable rounded-[18px] bg-white border border-line shadow-[var(--shadow-card)] overflow-hidden">
          {/* 헤더 */}
          <div className="px-5 py-4 text-white" style={{ background: "linear-gradient(135deg,#a50034,#76002a)" }}>
          ...
          <div className="px-5 py-4 space-y-4">
            {/* 제품 정보 */}
            <section>
              <h2 className="text-[12px] font-extrabold text-crimson mb-1.5">① 제품 정보</h2>
            ...
            {/* 진단 요약 */}
            <section>
              <h2 className="text-[12px] font-extrabold text-crimson mb-1.5">② 진단 요약</h2>
            ...
            {/* 증상 */}
            <section>
              <h2 className="text-[12px] font-extrabold text-crimson mb-1.5">③ 신고 증상 (심각도 1~5)</h2>
              {syms.length ? (
                <div className="space-y-1.5">
                  {syms.map((s) => (
                    <div key={s.type} className="flex items-center gap-2">
                      <span className="text-[12.5px] font-semibold text-[#241c20] w-20">{s.type}</span>
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((n) => (
                          <span key={n} className={`w-3.5 h-3.5 rounded-[4px] ${n <= s.severity ? "bg-crimson" : "bg-[#eadfd9]"}`} />
                        ))}
                      </div>
            ...
            {/* 접수 체크 */}
            <section>
              <h2 className="text-[12px] font-extrabold text-crimson mb-1.5">④ 방문 시 확인 요청</h2>
              {["증상 재현 및 원인 점검", "부품 보유/수리 가능 여부 안내", "수리비 견적 사전 안내"].map((t) => (
                <div key={t} className="flex items-center gap-2 text-[12.5px] text-[#241c20] py-0.5">
                  <Check size={13} className="text-crimson" strokeWidth={3} />{t}
                </div>
              ))}
            </section>

            {/* 안내 */}
            <p className="text-[10.5px] text-muted leading-relaxed border-t border-[#eadfd9] pt-3">
        ...
        <div className="no-print grid grid-cols-2 gap-2.5 mt-4">
          ...
          <button onClick={() => window.print()} className="flex items-center justify-center gap-1.5 rounded-2xl text-white font-extrabold py-3 text-[14px]" style={{ background: "linear-gradient(135deg,#a50034,#82002a)" }}>
