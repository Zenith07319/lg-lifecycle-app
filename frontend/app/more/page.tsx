      ...
      <div className="px-4 py-4">
        <div className="rounded-[20px] bg-surface border border-line shadow-[var(--shadow-card)] divide-y divide-line/70 overflow-hidden">
          {ITEMS.map(({ href, icon: Icon, label, desc, badge }) => (
            <Link key={href} href={href} className="flex items-center gap-3.5 px-4 py-3.5 active:bg-accent-soft">
              <div className="w-10 h-10 rounded-xl bg-accent-soft flex items-center justify-center shrink-0">
                <Icon size={19} className="text-accent" strokeWidth={2} />
              </div>
              ...
              {badge && cnt > 0 && (
                <span className="text-[10px] font-extrabold text-white bg-accent rounded-full min-w-5 h-5 px-1.5 flex items-center justify-center">{cnt}</span>
              )}
        ...
        <div className="rounded-[18px] bg-accent-soft border border-line p-4 mt-4 flex gap-3">
