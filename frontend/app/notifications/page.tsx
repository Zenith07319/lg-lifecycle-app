      ...
      <div className="px-4 py-4">
        {ready && list.length === 0 ? (
          <div className="text-center py-24 px-6">
            <div className="w-16 h-16 rounded-3xl bg-[#f0ece9] flex items-center justify-center mx-auto mb-4">
            ...
            <Link href="/diagnose" className="inline-block mt-5 rounded-2xl text-white font-extrabold px-5 py-3 text-[14px]" style={{ background: "linear-gradient(135deg,#a50034,#82002a)" }}>
              에어컨 진단하기
            </Link>
          </div>
        ) : (
          <div className="space-y-2.5">
            {list.map((a) => (
              <button key={a.id} onClick={() => router.push(`/result/${a.device.sessionId}`)}
                className={`w-full text-left rounded-[18px] border p-4 shadow-[var(--shadow-card)] flex gap-3 active:scale-[.99] transition-transform ${a.level === "warn" ? "bg-[#fff6f0] border-[#f3d9c8]" : "bg-surface border-line"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {a.level === "warn" && <span className="text-[9.5px] font-extrabold text-white bg-crimson rounded-full px-1.5 py-0.5">중요</span>}
                  </div>
